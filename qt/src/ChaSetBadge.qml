...
Rectangle {
    ...
    implicitHeight: isSm ? 16 : 22
    implicitWidth: Math.max(isSm ? 16 : 22, badgeText.implicitWidth + (isSm ? 12 : 20))
    radius: isSm ? 4 : 11
    ...
    color: {
        if (root.forceActive) {
            ...
            if (root.variant === "destructive") return Qt.rgba(0.725, 0.11, 0.11, 1.0)   // #b91c1c
        }
        if (root.forceHover) {
            ...
            if (root.variant === "destructive") return Qt.rgba(0.863, 0.149, 0.149, 1.0) // #dc2626
        }
        ...
        if (root.variant === "default") {
            ...
            return Qt.rgba(0.937, 0.267, 0.267, 1.0) // #ef4444
    ...
    Text {
        ...
        color: {
            if (root.variant === "default" || root.variant === "destructive") {
                return "#ffffff"
            } else if (root.variant === "secondary") {
