...
export function SidebarMenuSubButton({
  ...
  return useRender({
    ...
    props: {
      ...
      'data-active': isActive,
      className: cn(
        'flex h-7 min-w-0 w-full -translate-x-px items-center gap-2 overflow-hidden rounded-md px-2 text-left text-sidebar-foreground ring-sidebar-ring outline-hidden cursor-pointer group-data-[collapsible=icon]:hidden hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[size=md]:text-sm data-[size=sm]:text-xs data-active:bg-[var(--sidebar-selected)] data-active:text-[var(--sidebar-selected-foreground)] data-active:hover:bg-[var(--sidebar-selected)] data-active:hover:text-[var(--sidebar-selected-foreground)] data-active:[&>svg]:text-[var(--sidebar-selected-foreground)] [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0 [&>svg]:text-sidebar-accent-foreground',
        className,
      ),
      children: renderElement === children ? undefined : children,
      ...props,
